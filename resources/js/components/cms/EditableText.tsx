...
export default function EditableText({
    ...
    return (
        <div className={editMode && isAdmin ? 'group relative' : undefined}>
            ...
            {isAdmin && editMode && (
                <>
                    {/* Unsaved Changes Indicator - Always visible when has changes */}
                    <AnimatePresence>
                        {hasChanges && !isSaving && !saved && (
                            <motion.div
                                ...
                                className="absolute -bottom-7 left-0 z-50 rounded-md bg-amber-500 px-2.5 py-1 text-xs text-white font-semibold shadow-lg"
                            >
                                <span className="flex items-center gap-1.5">
                                    ...
                                    Unsaved - Click "Save" or click away
                                </span>
                            </motion.div>
                        )}
                    </AnimatePresence>
                    ...
                    <AnimatePresence>
                        {isSaving && (
                            <motion.div
                                ...
                                animate={{ opacity: 1, scale: 1 }}
                                exit={{ opacity: 0, scale: 0.8 }}
                                className="absolute -bottom-7 left-0 z-50 rounded-md bg-blue-500 px-2.5 py-1 text-xs text-white font-semibold shadow-lg"
                            >
                                <span className="flex items-center gap-1.5">
                    ...
                    <AnimatePresence>
                        {saved && (
                            <motion.div
                                ...
                                exit={{ opacity: 0, y: 8 }}
                                className="absolute -bottom-7 left-0 z-50 rounded-md bg-emerald-500 px-2.5 py-1 text-xs text-white font-semibold shadow-lg"
                            >
                                <span className="flex items-center gap-1.5">
