...
export default function EditableText({
    ...
    // Treat empty string from DB as "no content" so we show the default value
    const dbContent = props.sections?.[sectionKey]?.content;
    const resolvedFromDb = typeof dbContent === 'string' ? dbContent.trim() : dbContent;
    const initialValue = resolvedFromDb !== '' && resolvedFromDb != null ? dbContent! : (value ?? '');
    ...
    useEffect(() => {
        const newValue = resolvedFromDb !== '' && resolvedFromDb != null ? (dbContent ?? '') : (value ?? '');
        if (newValue !== text && !hasChanges) {
            setText(newValue);
        }
    }, [dbContent, value, text, hasChanges, resolvedFromDb]);
        ...
        <div className={editMode && isAdmin ? 'group relative' : undefined}>
            <div className="relative">
                <Tag
                    ...
                    className={`${className ?? ''} ${
                        isAdmin && editMode
                            ? `rounded-sm ring-2 transition-all outline-none ${
                                  hasChanges
                                      ? 'shadow-lg shadow-amber-500/20 ring-amber-500/80 hover:ring-amber-500'
