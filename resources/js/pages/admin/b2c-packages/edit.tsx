        ...
        <AdminPortalShell className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:py-10">
            ...
            <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <h1 className="mt-4 text-3xl font-semibold tracking-tight text-white sm:text-4xl">{pkg.name}</h1>
                    <p className={`mt-2 font-mono text-xs ${adminMuted}`}>Slug: {pkg.slug}</p>
                </div>
                <Link
                    href={`/admin/b2c-packages/${pkg.slug}/registrations`}
                    className={`${adminGhostBtn} inline-flex items-center border-[#fec901]/35 text-[#fec901] hover:bg-[#fec901]/10`}
                >
